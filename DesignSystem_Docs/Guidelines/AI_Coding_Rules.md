...
- 优先使用项目已有组件，禁止重复创建 Button、Input、Select、Table、Dialog、Pagination、Loading 等基础组件。
...
1. Breadcrumb
2. PageHeader
3. FilterArea / FormArea / DetailHeader
4. ContentArea
5. Pagination / FooterActions
...
- 输入框必须使用系统 Input。
- 下拉选择必须使用系统 Select。
- 日期选择必须使用系统 DatePicker / DateTimePicker。
- 表格必须使用系统 Table。
- 弹窗必须使用系统 Dialog。
- 分页必须使用系统 Pagination。
- 加载中必须使用系统 Loading。
- 空数据先按 Table / Pattern 的 empty 状态规则处理；是否独立 Empty 组件待确认。
...
- 禁止使用营销页 Hero 布局。
- 禁止使用装饰性插画、光斑、背景图。
- 禁止使用圆角超过 8px 的后台卡片；如果 Token 未确认圆角，标记为“待定”。
- 禁止使用大面积阴影制造悬浮卡片感。
- 禁止页面区块层层套卡片。
- 禁止使用过大的标题字号。
