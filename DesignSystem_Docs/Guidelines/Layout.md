...
本文件定义 BMS 后台页面的全局布局规则。页面级结构以 `Patterns/` 中的页面模板为准，本文件只约束通用布局壳、内容区组织方式和禁止事项。
...
- 内容区最大宽度以 Pattern 已确认值为准；未确认页面写 `待定`。
- 移动端或窄屏适配规则未在 Figma 中确认，统一写 `待定`。
